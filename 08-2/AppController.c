...
double AppController_timeForSortedArrayList_add(AppController *_this, UnsortedArrayList *aList, int aTestSize) {

}

double AppController_timeForSortedArrayList_removeMax(AppController *_this, UnsortedArrayList *aList, int aTestSize) {

}

double AppController_timeForSortedArrayList_min(AppController *_this, UnsortedArrayList *aList, int aTestSize) {
...
void AppController_showResults(AppController *_this, int aTestSize, double aTimeForAdd, double aTimeForMin,
    ...
    char results[255];
    sprintf_s(results, sizeof(results), "크기: %4d, 삽입: %6ld, 최소값얻기: %7ld, 최대값삭제: %7ld\n",
...
void AppController_initPerformanceMeasurement(AppController *_this) {
    ...
    _this->_testData = NewVector (int, ParameterSet_maxTestSize(_this->_parameterSet)); // 유의: 사용을 종료한 후, 이것을 소멸시킬 위치는?
