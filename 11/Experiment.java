public class Experiment {
    ...
    private long durationOfSingleSort(Sort<Integer> aSort, Integer[] aList, int size) {
        Timer timer = new Timer(); //Timer  instance  생성
        timer.start(); //시간 측정
        aSort.sort(aList, size);
        timer.stop(); //측정 완료

        return timer.duration(); //차이값 반환
    }
    ...
    public long[] durationsOfSort(Sort<Integer> aSort, Integer[] experimentList) {
        ...
        for (int step = 0; step < numberOfSteps; step++) {
            ...
            durations[step] = this.durationOfSingleSort(aSort, listForSorting, sortingSize); // 측정하여, 그 결과를 저장
