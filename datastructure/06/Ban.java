public class Ban extends UnSortedArrayList<Student> {
    ...
    private Student lowestRecursively(int left, int right) {
        if (left == right) { //초항
            ...
            if (lowestFromRight.compareTo(this.elementAt(left)) <= 0) { //하나씩 줄여나간다.
    ...
    private Student highestRecursively(int left, int right) {
        if (left == right) { //초항
            ...
            Student rightFromRights = highestRecursively(left + 1, right); //하나 씩 줄여나간다.
            if (rightFromRights.compareTo(this.elementAt(left)) > 0) { //비교
